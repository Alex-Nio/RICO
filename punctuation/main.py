...
if __name__ == "__main__":
    ...
    model_file_name = "/punctuation/Model_%s_h%d_lr%s.pcl" % (
    ...
    for epoch in range(starting_epoch, MAX_EPOCHS):
        ...
        for X, Y in get_minibatch(data.TRAIN_FILE, MINIBATCH_SIZE, shuffle=True):
            ...
            if iteration % 100 == 0:
                sys.stdout.write("PPL: %.4f; Speed: %.2f sps\n" % (np.exp(
                    total_neg_log_likelihood / total_num_output_samples), total_num_output_samples / max(time() - t0, 1e-100)))
                sys.stdout.flush()
        ...
        if ppl <= best_ppl:
            best_ppl = ppl
            models.save(net, model_file_name, learning_rate=learning_rate, validation_ppl_history=validation_ppl_history,
